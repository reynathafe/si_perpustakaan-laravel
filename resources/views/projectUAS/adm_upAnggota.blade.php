        ...
        <div class="row">
            <div class="col-md-12 col-sm-12">
                <div class="x_panel">
                    <div class="x_title">
                        <h2>Edit Anggota</h2>
                        <button type="button" class="btn btn-info btn-sm" style="float: right;"><a href="/Perpustakaan/Anggota" style="color: white;">Kembali</a></button>
                        <div class="clearfix"></div>
                    </div>
                    <div class="x_content">
                        @foreach ($kirim as $ed)
                        <form action="/Perpustakaan/Anggota/Update/{{ $ed->nim }}" method="POST"><br>
                            {{ csrf_field() }}
                            <div class="field item form-group">
                                <label class="col-form-label col-md-3 col-sm-3  label-align"><span class="required"></span></label>
                                <div class="col-md-6 col-sm-6">
                                    <input class="form-control" name="nim" type="hidden" value="{{ $ed->nim }}" required='required'/>
                                </div>
                            </div>
                            <div class="field item form-group">
                                <label class="col-form-label col-md-3 col-sm-3  label-align">Nama<span class="required">*</span></label>
                                <div class="col-md-6 col-sm-6">
                                    <input class="form-control" name="nama" type="text" value="{{ $ed->nama_ang }}" required='required'/></div>
                            </div>
                            <div class="field item form-group">
                                ...
                                <div class="col-md-6 col-sm-6">
                                    <select name="prodi" class="form-control" required='required'>
                                        <option value="{{ $ed->prodi }}">{{ $ed->prodi }}</option>
                            ...
                            <div class="field item form-group">
                                ...
                                <div class="col-md-6 col-sm-6">
                                    <select name="angk" class="form-control" required='required'>
                                        <option value="{{ $ed->angkatan }}">{{ $ed->angkatan }}</option>
                                        @for ($i = 2013; $i <= 2021; $i++)
